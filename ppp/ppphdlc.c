...
*** File: ppphdlc.c
***
*** Comments:  This file contains the PPPHDLC device functions needed
***            for communication.
***
************************************************************************
*END*******************************************************************/
...
{  /* Body */
   /* Start SPR P122-0266-12 remove all warnings from RTCS code. */
   /* PPPHDLC_STRUCT_PTR ppphdlc_ptr = (PPPHDLC_STRUCT_PTR)handle; */
   PPPHDLC_STRUCT_PTR ppphdlc_ptr = (PPPHDLC_STRUCT_PTR)((void _PTR_)handle);
   /* End SPR P122-0266-12 */
   ppphdlc_ptr->UP      = up;
...
{ /* Body */
   ...
   ppphdlc_ptr->STATS.ST_TX_TOTAL++;

   /* Verify the checksum */
   if (!PPPHDLC_CCITT16_check(&ppphdlc_ptr->FCS_SEND))  {
      ppphdlc_ptr->STATS.ST_TX_ERRORS++;
   } /* Endif */
...
{  /* Body */
   ...
   while (!pcb) {
      ...
      if (!pcb) {
         ppphdlc_ptr->STATS.ST_RX_DISCARDED++;
         PPPHDLC_delay(1);
      } /* Endif */
   } /* Endwhile */
   ...
   pcb->PRIVATE = pcb;
   /* Start SPR P122-0266-12 remove all warnings from RTCS code. */
   /* pcb->FRAG[0].LENGTH initialized later */
