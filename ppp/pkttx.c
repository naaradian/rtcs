...
*** File: pkttx.c
***
*** Comments:  This file contains the implementation of the PPP Tx
***            task.
***
************************************************************************
*END*******************************************************************/
...
{ /* Body */
   ...
   if (ppp_ptr->VALID != PPP_VALID) {
      ...
      PCB_free(pcb);
      return PPPERR_INVALID_PROTOCOL;
   } /* Endif */

   error = PPP_OK;
   PPP_mutex_lock(&ppp_ptr->MUTEX);
   if (!ppp_ptr->LINK_STATE) {
      error = PPPERR_LINK_NOT_OPEN;
   } else {
      /* SPR P122-0371-01 */
...
{ /* Body */
   ...
   if (queue == 0) {
      RTCS_task_exit(creator, PPPERR_OPEN_QUEUE_FAILED);
   } /* Endif */
