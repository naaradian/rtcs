...
*** File: dnsutil.c
***
*** Comments:  This file contains the functions used exclusively by the
...
{  /* Body */
   ...
   while ( DNS_Local_network_name[local_name_length] ) {
      ...
      */
      ...
      if ( error == DNS_INVALID_LOCAL_NAME ) {
         break;
      }/* Endif */
...
uchar _PTR_ DNS_parse_answer_name_to_dotted_form
   (
      ...
      switch ( answer_type ) {
         ...
         case DNS_PTR:
            if (query_type == DNS_PTR) {
               ...
               name_index++;
               addr_ptr = _mem_alloc_zero( sizeof( _ip_address ));
               if ( addr_ptr == NULL ) {
                  RTCS_log_error(ERROR_DNS, DNS_UNABLE_TO_ALLOCATE_MEMORY,
                                 0, 0, 0);
                  return( NULL );
               }/* Endif */
...
{  /* Body */
   ...
   */
   buffer_ptr = _mem_alloc_zero( DNS_MAX_UDP_MESSAGE_SIZE );
   if ( buffer_ptr == NULL ) {
      RTCS_log_error(ERROR_DNS, DNS_UNABLE_TO_ALLOCATE_MEMORY,
                     0, 0, 0);
      return( NULL );
   ...
   if ( local_sock == RTCS_HANDLE_ERROR ) {
      RTCS_log_error(ERROR_DNS, DNS_UNABLE_TO_OPEN_SOCKET,
                     0, 0, 0);
      _mem_free(buffer_ptr);
      return( NULL );
   }/* Endif */
   ...
   if (error != RTCS_OK) {
      RTCS_log_error(ERROR_DNS, DNS_UNABLE_TO_BIND_SOCKET,
                     0, 0, 0);
      _mem_free(buffer_ptr);
      return( NULL );
   } /* Endif */
   ...
   buffer_size = sizeof(DNS_MESSAGE_HEADER_STRUCT) + qname_size
    ...
    /* Remote address, DNS_Resolver currently uses port 1001 */
   ...
   if (error == RTCS_ERROR) {
      ...
      RTCS_log_error(ERROR_DNS, DNS_UNABLE_TO_SEND_QUERY,
                     0, 0, 0);
      _mem_free(buffer_ptr);
      return( NULL );
   }/* Endif */
   ...
   if ( !error || error == RTCS_ERROR ) {
      shutdown(local_sock, FLAG_ABORT_CONNECTION);
      RTCS_log_error(ERROR_DNS, DNS_NO_RESPONSE_FROM_RESOLVER,
                     0, 0, 0);
      _mem_free(buffer_ptr);
      return( NULL );
   } /* Endif */
   ...
   if ( temp_size == RTCS_ERROR ) {
     shutdown(local_sock, FLAG_ABORT_CONNECTION);
     RTCS_log_error(ERROR_DNS, DNS_PACKET_RECEPTION_ERROR,
                    0, 0, 0);
     _mem_free(buffer_ptr);
     return( NULL );
   }/* Endif */
