...
extern RTCSMIB_NODE MIBNODE_deviseRestart;
...
RTCSMIB_NODE MIBNODE_enterprises = {
   ...
   NULL,
   &MIBNODE_radian,//&MIBNODE_deviseRestart, //NULL,
   &MIBNODE_private,
...
{ /* Body */
 ...
 // varlen = 4;
   if (varlen) {
      varval = *(unsigned char _PTR_)varptr;
      varlen--;
	}
  if (varlen) {
     varval <<= 8;
...
/*FUNCTION*-------------------------------------------------------------
*
* Function Name   : RTCSMIB_uint_read
* Returned Value  : an ASN.1 INTEGER
* Comments  :  Read an unsigned INTEGER at varptr, varlen bytes long.
*
*END*-----------------------------------------------------------------*/


uint_32 RTCSMIB_uint_read
   (
      uchar_ptr   varptr,
      uint_32     varlen
   )
...
{ /* Body */
   ...
   mib->oidlen += (node->ID > 0x100) ? 1 : 0; //t my
   ASN1_READ_ID(mib,id);
      ...
      case RTCSMIB_OP_GETNEXT:
         ...
         if (node->ID < id) {
            ...
            if (node->CHILD) {
               ...
               node = node->CHILD;
               mib->oidlen += (node->ID > 0x7F) ? 2 : 1;
		mib->oidlen += (node->ID > 0x100) ? 1 : 0; //t
               continue;
            } /* Endif */
...
//comment to recommends of arc Ref No: JGod-1170-D5334     return (mib->errstat == SNMP_ERROR_noError);
//____________________________________________Ref No: JGod-1170-D5334
   mib->outbuf += mib->oidlen + 4;
      mib->outlen -= mib->oidlen + 4;
      mib->oidp = mib->outbuf;
      return FALSE;
...
/* End CR 916 */
   ...
   case RTCSMIB_NODETYPE_OID_FN:
      if (!oidptr) {
         ...
         varlen = 0;
         while (tempoidptr) {
	 //   varlen += (tempoidptr->ID > 0x7F) ? 1 : 1; //t
              varlen += (tempoidptr->ID > 0x7F) ? 2 : 1;
    	//	varlen += (tempoidptr->ID > 0xFF) ? 1 : 0; //t
//		varlen += (tempoidptr->ID > 0x7FF) ? 1 : 0; //t
//		varlen += (tempoidptr->ID > 0xffFF) ? 1 : 0; //t
