...
{ /* Body */
   ...
   /* For PPP 2.73 compatibility */
   if (!handle) {
      handle = _PPP273_handle;
   } /* Endif */

   /* Allocate state for this interface */
   ipcp_ptr = _mem_alloc(sizeof(IPCP_CFG_STRUCT));
   if (!ipcp_ptr) {
...
{ /* Body */
   ...
   error = PPP_send(handle, PPP_PROT_IP, pcb->PCB_PTR);
   RTCSLOG_PCB_FREE(pcb, error);
   if (error) {
      if_ptr->STATS.ST_TX_ERRORS++;
      RTCS_seterror(&if_ptr->STATS.ERR_TX, error, (uint_32)pcb);
