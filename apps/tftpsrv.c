...
*** File: tftpsrv.c
***
*** Comments:  This file contains the TFTP server implementation.
***
************************************************************************
*END*******************************************************************/
...
extern FT_CALL_STRUCT ft_tftp;
extern uint_32   TFTP_open  (pointer ft_data);
extern uchar_ptr TFTP_read  (uint_32_ptr size);
extern boolean   TFTP_eof   (void);
extern uint_32   TFTP_close (void);
...
{ /* Body */
//________________________________________________hier need init global variables

	ft_tftp.OPEN  = TFTP_open; 
	ft_tftp.READ  = TFTP_read;
  	ft_tftp.EOFT  = TFTP_eof;
   	ft_tftp.CLOSE = TFTP_close;
...
{ /* Body */
   ...
   tftpsrv_ptr = _mem_alloc_zero(sizeof(TFTPSRV_STATE_STRUCT));
   if (tftpsrv_ptr == NULL) {
...
//ok	for(;;) //t
   ...
   trans_ptr = _mem_alloc_zero(sizeof(TFTP_TRANS_STRUCT));
   if (trans_ptr == NULL) {
 ...
 //ok  for(;;){} //_________________________________________________________________t
   ...
   error = TFTPSRV_open_device(pkt_op, filename, filemode, &trans_ptr->FILE_PTR);
//no  for(;;){} //_________________________________________________________________t
   if (error) {
      switch (error) {
      case RTCS_EACCES:
   ...
   if (trans_ptr->SOCK == RTCS_SOCKET_ERROR) {
      TFTP_SEND(tftpsrv_ptr->SRV_SOCK, _tftp_error_srv, sockaddr);
      RTCS_io_close(trans_ptr->FILE_PTR);
      _mem_free(trans_ptr);
      return;
   } /* Endif */
 // no for(;;){} //_________________________________________________________________t
...
{ /* Body */
   ...
   case TFTPOP_DATA:
      ...
      if (block_num == trans_ptr->BLOCK) {
         ...
         write_len = RTCS_io_write(trans_ptr->FILE_PTR,
...
{ /* Body */
   ...
   datalen = RTCS_io_read(trans_ptr->FILE_PTR, (char_ptr)trans_ptr->SND.DATA, TFTP_DATA_SIZE);
   if (datalen < 0) {
      datalen = 0;
   } /* Endif */
...
{ /* Body */
 ...
 //t   RTCS_io_close(trans_ptr->FILE_PTR);
   _io_fclose(trans_ptr->FILE_PTR); //my ? maybe need correct _io_close? make io_close1?
   shutdown(trans_ptr->SOCK, 0);
