...
{ /* Body */
   ...
   error = sendto(control_ptr->RES_SOCKET, control_ptr->RES_BUFFER, buffer_size, 0,
                  &raddr, sizeof(raddr));
   if (error == RTCS_ERROR) {
      RTCS_log_error(ERROR_DNS, DNS_UNABLE_TO_SEND_QUERY, 1, 0, 0);
      error = DNS_UNABLE_TO_SEND_QUERY;
   } else {
      error = DNS_OK;
   } /* Endif */
...
uint_32 DNS_return_UDP_response_to_query
   (
...
{ /* Body */
   ...
   if (error == RTCS_ERROR) {
      RTCS_log_error(ERROR_DNS, DNS_UNABLE_TO_SEND_QUERY, 2, 0, 0);

      DNS_free_internal_query( current_query_ptr );
      return( DNS_UNABLE_TO_SEND_QUERY );
   } /* Endif */
...
uint_32 DNS_get_best_name_server_for_query
   (
   DNS_INTERNAL_QUERY_STRUCT _PTR_ query_ptr,
   DNS_CONTROL_STRUCT         _PTR_ control_ptr
   )
...
{ /* Body */
  ...
  temp_ptr = control_ptr->LOCAL_SLIST_PTR;

  query_ptr->SERVER_IPADDR = temp_ptr->IPADDR;
...
*END*-----------------------------------------------------------------*/
 ...
 DNS_INTERNAL_QUERY_STRUCT _PTR_ DNS_get_next_expired_query
   (
...
void DNS_send_no_query_error_response
   (
   DNS_CONTROL_STRUCT _PTR_ control_ptr,
   uint_16                 error_code,
...
{ /* Body */
   ...
   if (error == RTCS_ERROR) {
      RTCS_log_error(ERROR_DNS, DNS_UNABLE_TO_SEND_QUERY, 3, 0, 0);
   } /* Endif */
...
uint_32 DNS_fill_generic_response_buffer
   (
   uchar                     _PTR_ buffer,
   DNS_INTERNAL_QUERY_STRUCT _PTR_ query_ptr,
   uint_16                         error_code
...
{ /* Body */
   ...
   if (error == RTCS_ERROR) {
      RTCS_log_error(ERROR_DNS, DNS_UNABLE_TO_SEND_QUERY, 4, 0, 0);
   } /* Endif */
...
void DNS_check_outstanding_queries
   (
   DNS_CONTROL_STRUCT _PTR_ control_ptr
   )
   ...
   current_ptr = DNS_get_next_expired_query( &control_ptr->RES_QUERY_HEAD_PTR );
      ...
      while ( current_ptr ) {
         ...
         */
         ...
         error = DNS_get_best_name_server_for_query( current_ptr,
                                                     control_ptr );
         if ( error != DNS_OK ) {
            DNS_send_error_response( control_ptr->RES_BUFFER, current_ptr,
...
uint_16 DNS_check_packet_for_name
   (
   uchar _PTR_ buffer_ptr,
   uint_32     buffer_size,
   uchar _PTR_ name_ptr
...
uint_32 DNS_fill_answer_section_of_buffer
   (
   DNS_CONTROL_STRUCT   _PTR_ control_ptr,
...
uint_32 DNS_fill_response_buffer_with_rr_data
   (
   DNS_CONTROL_STRUCT   _PTR_ control_ptr,
...
uint_32 DNS_rr_canonical_data_exists
   (
...
DNS_RESPONSE_RR_MIDDLE_STRUCT _PTR_ DNS_assign_answer_middle_ptr
   (
...
void DNS_assign_answer_pointers
   (
   uchar                               _PTR_ buffer,
...
{ /* Body */
   ...
   temp_ptr = (uchar _PTR_)_mem_alloc_zero( DNS_MAX_CHARS_IN_NAME );
   if ( temp_ptr == NULL ) {
   ...
   *local_addr_buffer = (uchar _PTR_)_mem_alloc_zero( total_size );
...
{ /* Body */
   ...
   if ( error != DNS_OK ) {
      ...
      if ( ntohc(domain_name) == '\0' ) {
         *local_name_buffer = (uchar _PTR_)_mem_alloc_zero( 1 );
      ...
      fill_ptr = (uchar _PTR_)_mem_alloc_zero( DNS_MAX_CHARS_IN_NAME );
      if ( fill_ptr == NULL ) {
      ...
      *local_name_buffer = (uchar _PTR_)_mem_alloc_zero( total_length );
      if ( *local_name_buffer == NULL ) {
...
{ /* Body */
   ...
   if ( new_query_ptr == NULL ) {
      RTCS_log_error( ERROR_DNS, DNS_UNABLE_TO_ALLOCATE_MEMORY, 0, 0, 0);
   ...
   if ( error == DNS_INVALID_NAME || error == DNS_INVALID_IP_ADDR ) {
...
DNS_INTERNAL_QUERY_STRUCT _PTR_ DNS_retrieve_query
   (
   uint_16                               id,
   DNS_INTERNAL_QUERY_STRUCT _PTR_ _PTR_ query_head_ptr
   )
...
*
*END*-----------------------------------------------------------------*/
void DNS_remove_last_entry_from_cache
   (
   DNS_CONTROL_STRUCT _PTR_ control_ptr
   )
{ /* Body */
...
uint_32 DNS_retry_query_with_CNAME
   (
   DNS_CONTROL_STRUCT         _PTR_ control_ptr,
   DNS_INTERNAL_QUERY_STRUCT _PTR_ query_ptr,
...
uint_32 DNS_search_response_for_referral
   (
   DNS_CONTROL_STRUCT         _PTR_ control_ptr,
   DNS_INTERNAL_QUERY_STRUCT _PTR_ query_ptr
   )
...
uint_32 DNS_handle_internal_query
   (
   DNS_CONTROL_STRUCT         _PTR_ control_ptr,
   DNS_INTERNAL_QUERY_STRUCT _PTR_ query_ptr
   )
...
*END*-----------------------------------------------------------------*/
 ...
 uint_32 DNS_handle_UDP_response
   (
   DNS_CONTROL_STRUCT   _PTR_ control_ptr
   )
...
void DNS_refresh_root_servers_task
   (
   uint_32 temp_ptr
   )
{ /* Body */
...
*END*-----------------------------------------------------------------*/
 ...
 uint_32 DNS_refresh_root_servers
   (
   DNS_CONTROL_STRUCT _PTR_ control_ptr
   )
{ /* Body */
...
{ /* Body */
   ...
   */
   ...
   if ( error == (int_32)RTCS_SOCKET_ERROR ) {
      RTCS_task_exit(creator, DNS_INITIALIZATION_FAILURE);
   } /* Endif */
   ...
   while ( TRUE ) {
      ...
      */
      ...
      if (first_root_ptr->TTL <= difference
         ...
         if ( error != DNS_OK ) {
            RTCS_log_error(ERROR_DNS, DNS_UNABLE_TO_UPDATE_ROOT_SERVERS, 0, 0, 0);
         } /* Endif */
      } else {
         RTCS_mutex_unlock(&control_ptr->CACHE_MUTEX);
      ...
      */
      ...
      if (control_ptr->RES_BUFFER_SIZE == RTCS_ERROR) {
         RTCS_log_error(ERROR_DNS, DNS_SOCKET_RECV_ERROR, 0, 0, 0);
         continue;
      } /* Endif */
      message_head_ptr = (DNS_MESSAGE_HEADER_STRUCT_PTR)control_ptr->RES_BUFFER;
