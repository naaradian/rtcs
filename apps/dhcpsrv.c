...
*** File: dhcpsrv.c
***
*** Comments:  This file contains the DHCP Server for RTCS.
***
************************************************************************
*END*******************************************************************/
...
DHCPSRV_ADDR_STRUCT_PTR DHCPSRV_lease_stop
   (
      DHCPSRV_ADDR_STRUCT_PTR _PTR_ addrhead
   )
{ /* Body */
...
{ /* Body */
   ...
   options = _mem_alloc_system(sizeof(DHCPSRV_OPTIONS_STRUCT) + optlen);
   if (!options) {
   ...
   for (; ipnum; ipnum--, ipstart++) {
      addr = _mem_alloc_system(sizeof(DHCPSRV_ADDR_STRUCT));
      if (!addr) {
         if (options->COUNT == 0) {
            _mem_free(options);
         } /* Endif */
...
DHCPSRV_ADDR_STRUCT_PTR _PTR_ DHCPSRV_find_clientid
   (
      DHCPSRV_ADDR_STRUCT_PTR _PTR_ addrhead,
      uchar_ptr                     cid_ptr,
      uint_32                       cid_len,
...
DHCPSRV_ADDR_STRUCT_PTR _PTR_ DHCPSRV_find_addr
   (
      DHCPSRV_ADDR_STRUCT_PTR _PTR_ addrhead,
      _ip_address                   addr
   )
...
DHCPSRV_ADDR_STRUCT_PTR _PTR_ DHCPSRV_update_clientid
   (
      DHCPSRV_ADDR_STRUCT_PTR _PTR_ addrhead,
      uchar_ptr                     cid_ptr,
      uint_32                       cid_len
...
uint_32 DHCPSRV_set_config_flag_on
   (
...
uint_32 DHCPSRV_set_config_flag_off
   (
      uint_32	flags /* [IN]   flags to govern behaviour of dhcp server */
   )
