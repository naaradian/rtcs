...
int_32 FTP_receive_message      (uint_32, FILE_PTR);
int_32 FTP_receive_message_pasv (uint_32, FILE_PTR, _ip_address _PTR_, uint_16 _PTR_);
void   FTP_execute_command      (uint_32, boolean, FILE_PTR);
...
{ /* Body */
   ...
   message = FTP_receive_message(ctrl_sock, ctrl_fd);
...
{ /* Body */
   char        message_buffer[BUF_SIZE];
   uint_32     i, comma_count = 0, chars_left = 0;
   boolean     message_found = FALSE;
   int_32      message = 0, digit_count = 0, count = 0, p;
   ...
   while (1) {
      ...
      if ((digit_count == 0) && (isdigit(message_buffer[i]))) {
         ...
         if (isdigit(message_buffer[i])) {
            p *= 10;
            p += (message_buffer[i] - '0');
         } else if (message_buffer[i] == ',') {
            comma_count++;
            if (comma_count <= 4) {
               ipaddr <<= 8;
...
{ /* Body */
   ...
   return FTP_receive_message(ctrl_sock, ctrl_fd);
   ...
   */
   ...
   case 2:
      ...
      if (mode == 2) {
         ...
         sprintf(command_str, "PORT %ld,%ld,%ld,%ld,%hd,%hd\r\n",
                 ...
                 (listen_addr.sin_port        >>  8) & 0xFF,
                  listen_addr.sin_port               & 0xFF);
...
{ /* Body */
   ...
   if (sending) {
      for (;;) {
         length = read(data_fd, buff, BUF_SIZE);
         if (length < 0) break;
         send(data_sock, buff, length, 0);
      } /* Endfor */

   } else {
      for (;;) {
         length = recv(data_sock, buff, BUF_SIZE, 0);
         if (length < 0) break;
