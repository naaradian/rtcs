...
#define TCP_MAX_CONNECTIONS      0  /* Maximum number of simultaneous
                                       connections allowed.  Define as
                                       0 for no limit. */
#endif

#ifndef TCP_MAX_HALF_OPEN
#define TCP_MAX_HALF_OPEN        0  /* Maximum number of simultaneous
                                       half open connections allowed.
                                       Define as 0 to disable the
                                       SYN attack recovery feature. */
#endif
...
typedef struct tcb_parm {
   ...
   struct TCB _PTR_ TCB_PTR;     /* TCP Context (TCB) */
...
typedef struct {
    ...
    struct TCB _PTR_ tcb;   /* TCB which contains this timer */
...
typedef struct TCB {
   ...
   struct TCB _PTR_     next;       /* TCP server maintained list of TCB's */
   ...
   /* Start CR 1116 */
#if 0
   int_16    usecount;       /* for clients; initially 1 when TCB is opened;
                                TCB will not be deallocated while
                                usecount != 0; once usecount reaches 0,
                                this means all clients have relinquished
                                access to the TCB, and thereafter may not
                                refer (or make requests) to this TCB;
                                usually, a task that "owns" or has control
                                of a link (a single usecount increment) may
                                add another by calling TcpLink(),
                                on behalf of another task (perhaps the
                                receive and send tasks); TcpRelease() is
                                used to decrement (unlink) this value. */
#endif
   /* End CR */
   ...
   struct TCB _PTR_     LISTEN_TCB;  /* The listening TCB which was cloned */
   struct TCB _PTR_     NOSOCK_NEXT; /* A linked list of TCBs spawned from
                                        listening TCB */
   uint_32   tcb_spawn_time; /* keep the time when tcb is created */

   /* Start CR 1004 */
   /* Required for sending after the socket is freed */
...
typedef struct tcp_cfg {
   ...
   TCB_STRUCT_PTR  HALF_OPEN_TCB_LIST[TCP_MAX_HALF_OPEN];
