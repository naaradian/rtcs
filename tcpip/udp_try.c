...
*** File: udp.c
***
*** Comments:  This file contains the implementation of the User
...
***   Feb  3/93   1.1      Added error check to _mem_free calls
***   Nov 17/93   2.10     Fixed UDP checksum per change to IP_Sum;
***                        Fixed UDP_Send to return OK if IP_Send fails
***   Mar 10/97   2.60     Medium Independance
***   Oct  6/98   2.72     Socket Task Elimination
***   Dec 21/98   2.73     IGMP support
***   Jul 12/00   2.77     Added RTCS_MSG_PEEK option on recvfrom()
***   Sep 15/00   2.80     Added send, recv and connect for UDP
***
************************************************************************
*END*******************************************************************/
...
{ /* Body */
   ...
   UDP_cfg_ptr = _mem_alloc_zero(sizeof(UDP_CFG_STRUCT));
...
{ /* Body */
   ...
   ucb = _mem_alloc(sizeof(UCB_STRUCT));
   if (!ucb) {
...
{ /* Body */
   ...
   */
   ...
   if (pcb_ptr) {
      ...
      */
      parms->ipaddress = IP_source(pcb_ptr, -2);
      parms->udpport   = UDP_source(pcb_ptr, -1);
...
{ /* Body */
   ...
   if (parms->udpport == 0) {
      UDP_cfg_ptr->STATS.ST_TX_TOTAL++;
      UDP_cfg_ptr->STATS.ST_TX_DISCARDED++;
   ...
   if (pcb == NULL) {
      UDP_cfg_ptr->STATS.ST_TX_TOTAL++;
      UDP_cfg_ptr->STATS.ST_TX_MISSED++;
   ...
   if (nonblock) {
      ...
      udpbuffer = _mem_alloc_system(parms->udpword);
      if (!udpbuffer) {
         UDP_cfg_ptr->STATS.ST_TX_TOTAL++;
         UDP_cfg_ptr->STATS.ST_TX_MISSED++;
      ...
      error = RTCSPCB_frag_wfree(pcb, parms->udpword, udpbuffer);
      if (error) {
         UDP_cfg_ptr->STATS.ST_TX_TOTAL++;
         UDP_cfg_ptr->STATS.ST_TX_ERRORS++;
         RTCS_seterror(&UDP_cfg_ptr->STATS.ERR_TX, error, (uint_32)pcb);
         _mem_free(udpbuffer);
      ...
      error = RTCSPCB_frag(pcb, parms->udpword, parms->udpptr);
      if (error) {
         UDP_cfg_ptr->STATS.ST_TX_TOTAL++;
         UDP_cfg_ptr->STATS.ST_TX_ERRORS++;
         RTCS_seterror(&UDP_cfg_ptr->STATS.ERR_TX, error, (uint_32)pcb);
...
{ /* Body */
   ...
   error = RTCSPCB_insert(pcb_ptr, sizeof(UDP_HEADER));
   if (error) {
      UDP_cfg_ptr->STATS.ST_TX_ERRORS++;
      RTCS_seterror(&UDP_cfg_ptr->STATS.ERR_TX, error, (uint_32)pcb_ptr);
...
{ /* Body */
   ...
   */
   if (len < sizeof(UDP_HEADER)) {
      UDP_cfg_ptr->STATS.ST_RX_DISCARDED++;
      UDP_cfg_ptr->STATS.ST_RX_SMALL_DGRAM++;
   ...
   if (RTCSPCB_SIZE(pcb_ptr) < len) {
      UDP_cfg_ptr->STATS.ST_RX_DISCARDED++;
      UDP_cfg_ptr->STATS.ST_RX_SMALL_PKT++;
      RTCSLOG_PCB_FREE(pcb_ptr, RTCSERR_UDP_BAD_HEADER);
      RTCSPCB_free(pcb_ptr);
      return;
   } /* Endif */
   ...
   */
   if (port == 0) {
      UDP_cfg_ptr->STATS.ST_RX_DISCARDED++;
      UDP_cfg_ptr->STATS.ST_RX_BAD_PORT++;
      RTCSLOG_PCB_FREE(pcb_ptr, RTCSERR_UDP_BAD_HEADER);
      RTCSPCB_free(pcb_ptr);
      return;
   } /* Endif */
   ...
   */
   ...
   ipsource = IP_source(pcb_ptr, -1);
   ipdest = IP_dest(pcb_ptr, -1);
   ...
   */
   if (!ucb_ptr) {
      UDP_cfg_ptr->STATS.ST_RX_DISCARDED++;
      UDP_cfg_ptr->STATS.ST_RX_NO_PORT++;
   ...
   */
   if (ntohs(packet->CHECKSUM) && !ucb_ptr->BYPASS_RX) {
      ...
      if (sum != 0xFFFF) {
         UDP_cfg_ptr->STATS.ST_RX_DISCARDED++;
         UDP_cfg_ptr->STATS.ST_RX_BAD_CHECKSUM++;
   ...
   */
   ...
   if (error) {
      UDP_cfg_ptr->STATS.ST_RX_ERRORS++;
      RTCS_seterror(&UDP_cfg_ptr->STATS.ERR_RX, error, (uint_32)pcb_ptr);
      RTCSLOG_PCB_FREE(pcb_ptr, error);
      RTCSPCB_free(pcb_ptr);
      return;
   } /* Endif */
...
{ /* Body */
   ...
   */
   ...
   if (req_ptr) {
      ...
      req_ptr->ipaddress = IP_source(pcb_ptr, -2);
      req_ptr->udpport   = UDP_source(pcb_ptr, -1);
      ...
      UDP_cfg_ptr->STATS.ST_RX_MISSED++;
      RTCSLOG_PCB_FREE(pcb_ptr, RTCS_OK);
      RTCSPCB_free(pcb_ptr);
      return;
   ...
   */
      ...
      */
      SOCK_select_signal(ucb_ptr->SOCKET);
...
uint_16 UDP_source
   (
      ...
      int_32         layer    /* [IN] UDP layer, relative to current */
   )
{ /* Body */
   uchar_ptr srcptr = ((UDP_HEADER_PTR)pcb->LAYER[pcb->CL + layer])->SRC_PORT;
