...
{ /* Body */
   ...
   if ( timeq->curlist == qhead_ptr ) { /* currently active timer */
      ...
      if ( delay > delta ) {           /* Longer delay requested */
         timeq->reload = reload;
         timeq->count = 0;
         return TCP_Timer_advance(timeq, delay - timeq->delay, qhead_ptr);
...
{ /* Body */
   ...
   */
   if ( timeq->curlist != qhead_ptr ) {
      return TCP_Timer_start(timeq, delta, 0, qhead_ptr);
   } /* Endif */
...
{ /* Body */
   ...
   return TCP_Timer_start(timeq, delay, 0, qhead_ptr);
