               ...
               (tcb->rcvevents & TCPO_PUSH) == 0)
            {
               ...
               */
               ...
               if ( tcp_cfg->Rchunkfree != NULL ) {
                  ...
                  chunk = (Rchunk _PTR_)_mem_alloc_zero(sizeof(Rchunk));
               } /* Endif */

               if ( chunk == NULL ) {
                  tcp_cfg->STATS.ST_RX_MISSED++;
...
{ /* Body */
   ...
   */
   ...
   if (pcb->FRAGUSED) {
      ...
      /* Start SPR P122-0266-25 remove all warnings from RTCS code. */
      /*data = pcb->PCB_PTR->FRAG[1].FRAGMENT + truncate; */

      pcb_frag_ptr = pcb->PCB_PTR->FRAG;
      pcb_frag_ptr++;
      /* pointer to new data */
      data  = pcb_frag_ptr->FRAGMENT + truncate;
      /* End SPR P122-0266-25. */
   } /* Endif */
   ...
   */
   ...
   if ( LT32(dataseq, tcb->rcvnxt) ) { /* first byte past rcvnxt */
