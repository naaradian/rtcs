   ...
   */
   ...
   if ( tcb->usecount != 0 ) {
      RTCS_log_error( ERROR_TCP,
                           RTCSERR_TCP_BAD_STATE_FOR_REL,
                           (uint_32)tcb->state,
                           (uint_32)tcb->usecount,
                           (uint_32)tcb );
      tcb->usecount = 0;
   } /* Endif */
#endif
...
{ /* Body */
   ...
   if (tcb->VALID == TCB_VALID_ID) {
      /* Start CR 1004 */
      /* Copy TX options. They may still be needed */
      tcb->TX = ((SOCKET_STRUCT_PTR)tcb->SOCKET)->LINK_OPTIONS.TX;
      /* End CR */
      tcb->SOCKET = 0;
   } /* Endif */
