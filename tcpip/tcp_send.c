...
{ /* Body */
   ...
   */
   ...
   /* Start CR 1004 */
   /* Use SOCKET TX options, or TCB copy from when socket was freed */
   if (tcb->SOCKET) {
      pcb->LINK_OPTIONS.TX = ((SOCKET_STRUCT_PTR)tcb->SOCKET)->LINK_OPTIONS.TX;
   } else {
   ...
   DEBUGTCP(printf("\nTCP_SendPkt: sport=%d, dport=%d, seq=%X, ack=%X, flags=%X, wndw=%d ",
      ntohs(seg->source_port), ntohs(seg->dest_port),
      ntohl(seg->seq), ntohl(seg->ack),
   ...
   tcp_cfg->STATS.ST_TX_TOTAL++;
   if ( err != RTCS_OK ) {
      tcb->lasterror = err;
      TCP_Event(tcb, TCPS_SOFTERROR);
      tcp_cfg->STATS.ST_TX_ERRORS++;
      tcp_cfg->STATS.ST_TX_DISCARDED++;
   } /* Endif */
...
{ /* Body */
   ...
   RTCSLOG_PCB_ALLOC(pcb);
   RTCSPCB_insert(pcb, sizeof(TCP_HEADER));
   RTCSLOG_PCB_WRITE(pcb, RTCS_LOGCTRL_PROTO(IPPROTO_TCP), 0);
