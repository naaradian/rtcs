...
*** File: iproute.c
***
*** Comments:  This file contains the interface to the IP routing
***            table.
***
************************************************************************
*END*******************************************************************/
...
/* Start CR 1133 */
void IP_route_add_indirect_sort
   (
      pointer       head,
      pointer       new_node
   )
...
{ /* Body */
   ...
   _ip_address _PTR_       gateway_ptr = data;
   /* Start CR 1133 */
   IP_CFG_STRUCT_PTR       IP_cfg_ptr = RTCS_getcfg(IP);
   IP_ROUTE_FN_PTR         fn;
   /* End CR */
...
{ /* Body */
   ...
   if ((ipdest == INADDR_BROADCAST) || IN_MULTICAST(ipdest)) {
      /* Accept broadcasts and multicasts */
   ...
   if (indirect && testdata->check_gateway) {
      do {
          if (indirect->FLAGS & RTF_UP) {
            ...
            /* Start CR 1133 */
            /* Use a different gateway next time */
            /* route->INDIRECT = route->INDIRECT->NEXT; */
            /* End CR */
...
//printfp("\n\r IP_route");
//printfpd(" %lX", (unsigned long)ipdest);
...
//printfp("!testdata.check_gateway");
   } /* Endif */
