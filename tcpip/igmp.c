...
{ /* Body */
   ...
   IGMP_cfg_ptr = _mem_alloc_zero(sizeof(IGMP_CFG_STRUCT));
   if (!IGMP_cfg_ptr) {
...
{ /* Body */
   ...
   IGMP_cfg_ptr->STATS.ST_TX_TOTAL++;
   ...
   if (ipif == NULL) {
      IGMP_cfg_ptr->STATS.ST_TX_ERRORS++;
      return FALSE;
   } /* Endif */

   pcb = RTCSPCB_alloc_send();
   if (pcb == NULL) {
      IGMP_cfg_ptr->STATS.ST_TX_MISSED++;
      return FALSE;
   } /* Endif */

   RTCSLOG_PCB_ALLOC(pcb);

   error = RTCSPCB_insert(pcb, sizeof(IGMP_HEADER));
   if (error) {
   ...
   IGMP_cfg_ptr->STATS.ST_TX_REPORT++;    /* WORK: not always true for IGMP_V2 */
...
{ /* Body */
   ...
   IGMP_cfg_ptr->STATS.ST_RX_TOTAL++;

   header = (IGMP_HEADER_PTR)RTCSPCB_DATA(pcb);

   /* check if length >= sizeof(IGMP_HEADER) */
   if (RTCSPCB_SIZE(pcb) < sizeof(IGMP_HEADER)) {
      IGMP_cfg_ptr->STATS.ST_RX_DISCARDED++;
      IGMP_cfg_ptr->STATS.ST_RX_SMALL_DGRAM++;
   ...
   if (IP_Sum_PCB(0, pcb) != 0xFFFF) {
      IGMP_cfg_ptr->STATS.ST_RX_DISCARDED++;
      IGMP_cfg_ptr->STATS.ST_RX_BAD_CHECKSUM++;
   ...
   case IGMPTYPE_QUERY:
      IGMP_cfg_ptr->STATS.ST_RX_QUERY++;
   ...
   case IGMPTYPE_V2_REPORT:
      IGMP_cfg_ptr->STATS.ST_RX_REPORT++;
      IGMP_rcv_report(pcb->IFSRC, ntohl(header->GROUP_ADDRESS));
      break;
   default:
      IGMP_cfg_ptr->STATS.ST_RX_BAD_TYPE++;
   } /* Endswitch */
