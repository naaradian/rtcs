   ...
   */
   ...
   if ( buf != NULL && size != 0 ) {
      ...
      if ( buf->use_cbuf ) {
         ...
         ** to point to it.
         */
         RTCSPCB_frag(pcb_ptr, size, buf->data + offset);
         ...
         */
         ...
         if (size > ring_tail) {
            RTCSPCB_frag(pcb_ptr, ring_tail, tcb->sndringbuf + ring_offs);
            RTCSPCB_frag(pcb_ptr, size - ring_tail, tcb->sndringbuf);
         } else {
            RTCSPCB_frag(pcb_ptr, size , tcb->sndringbuf + ring_offs);
         } /* Endif */
