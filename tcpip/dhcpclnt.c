...
*** File: dhcpclnt.c
***
*** Comments:  This file contains the implementation of the DHCP client
...
{ /* Body */
   ...
   dhcp_ptr = _mem_alloc_zero(sizeof(DHCP_CLNT_STRUCT) + buff_len);
...
{ /* Body */
   ...
   /*
   ** SPR P122-0216-01
   ** DHCP client doesn't respond to unicast OFFERs.  Temporary
   ** solution is to request that the server broadcast all replies.
   */
   htons(header_ptr->FLAGS, DHCPFLAG_BROADCAST);
   /* END SPR P122-0216-01 */
...
{ /* Body */
   ...
   RTCSPCB_frag(pcb_ptr, dhcp_ptr->CURRENT_PACKET_SIZE, dhcp_ptr->PACKET);
...
{ /* Body */
   ...
   if (RTCSPCB_SIZE(pcb_ptr) < sizeof(DHCP_HEADER) + 4) {
      *error = DHCPCLNT_PACKET_SIZE_ERROR;
      return 0;
   } /* Endif */
...
{ /* Body */
   /* Our request was refused. Start over from scratch */
