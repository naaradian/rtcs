...
*** File: rip.c
***
*** Comments:  This file contains the implementation of RIPv1 and v2,
...
{ /* Body */
    ...
    /* allocate the memory */
    cfg = _mem_alloc_zero(sizeof(*cfg));
...
{ /* Body */
   ...
   err = RTCSPCB_frag_wfree(pcb, len, data);
   if (err) {
       _mem_free(data);
...
{ /* Body */
    ...
    /* INADDR_ANY ?? so how can i send a default route? */
    return 1;
} /* Endbody */
   ...
   do {
      do {
         ...
         do {
            ...
            if (!testdata->pkt)  {
               testdata->pkt = testdata->curs = _mem_alloc_system(RIP_MAX_PKTLEN);
...
{ /* Body */
    ...
    /* allocate the packet memory */
    pkt = _mem_alloc_system(RIP_MAX_PKTLEN);
