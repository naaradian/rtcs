...
extern unsigned long keephandle2;
extern unsigned long keephandle3;

/*FUNCTION*-------------------------------------------------------------
*
*  Function Name  : IPIF_add1t
*  Parameters     :
*
...
*
*  Comments       :
*        Registers a hardware interface with RTCS. used for not allocate memory for ihandle if is used restart rtcs
*
*END*-----------------------------------------------------------------*/
...
//	 _mem_free(ipif); // previons make free
    	  ipif = _mem_alloc_zero(IPIFALLOC_SIZE * sizeof(IP_IF));
	   //   ipif =  (IP_IF_PTR) keephandle;
//	     keephandle = (unsigned long)ipif; //101215
  //       keephandle = 0x1234;
 ...
 //hangt   ipif =  (IP_IF_PTR) keephandle2; //101216

// keephandle2 = (unsigned long)ipif; //101215
//	keephandle2++;
 if (!ipif) {
   ...
   error = ipif->DEVICE.OPEN(ipif); //???? mem
   ...
   IGMP_ipif_add(ipif); //????? mem

//hangt!!!!  ipif =  (IP_IF_PTR) keephandle3; //101216
//  keephandle3 = (unsigned long)ipif;
   parms->ihandle = (_rtcs_if_handle)parms->ihandle;

  // keephandle3 = (unsigned long)12345; //for test
...
//  parms->ihandle =  (_rtcs_if_handle)keephandle3; //101217
   keephandle3 = (unsigned long)ipif; //101215
