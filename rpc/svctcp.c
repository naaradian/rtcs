...
*** ARC International.
***
*** Comments:  This file contains the implementation of the TCP/IP
...
{ /* Body */
   ...
   uint_16 llen = sizeof(laddr);
      ...
      */
   xprt = _mem_alloc_zero(sizeof(SVCXPRT));
   if (xprt == NULL) {
      ...
      */
   sc = _mem_alloc_zero(sizeof(SVC_CONN));
   if (sc == NULL) {
      fprintf(stderr, "svctcp_create: out of memory\n");
      goto create_fail;
   } /* Endif */
...
{ /* Body */
   ...
   SVC_TCP_PTR  st = NULL;
      ...
      ** Allocate a transport handle
      */
   xprt = _mem_alloc_system_zero(sizeof(SVCXPRT));
   if (xprt == NULL) {
      ...
      ** Allocate a private handle
      */
   st = _mem_alloc_system_zero(sizeof(SVC_TCP));
   if (st == NULL) {
      fprintf(stderr, "svctcp_fork: out of memory\n");
      _mem_free(xprt);
      return (NULL);
